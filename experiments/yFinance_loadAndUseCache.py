...
if __name__ == "__main__":
    ...
    prices = yFinanceHandler.loadFromRamOrAsyncFetchHistoryPricesOf(symbols_df["symbol"].tolist(), datetime(2024, 1, 1), datetime(2025, 7, 17), "1d")
    ...
    # Do again but with cache
    start_time = time.time()
    prices = yFinanceHandler.loadFromRamOrAsyncFetchHistoryPricesOf(symbols_df["symbol"].tolist(), datetime(2025, 1, 5), datetime(2025, 7, 1), "1d")
    for symbol, price in zip(symbols_df["symbol"].tolist(), prices):
        # print(f"\n{symbol}: {price}")
        pass
    end_time = time.time()
